// Auth screen — sign in / sign up with captcha

const AuthScreen = ({ onAuthed, captchaVariant }) => {
  const [mode, setMode] = React.useState("signin");
  const [email, setEmail] = React.useState("alex@acme.co");
  ...
  const social = (provider) => {
    ...
    setTimeout(() => {
      setLoading(false);
      onAuthed({ email: provider === "google" ? "alex@gmail.com" : "alex@users.noreply.github.com", name: "Alex Rivera", provider });
    }, 600);
  };
      ...
      <div style={{display: "flex", flexDirection: "column", alignItems: "center", gap: 18, width: "100%"}}>
        ...
        <form className="auth-card glass-strong" onSubmit={submit}>
          ...
          <div className="social-row">
            <button type="button" className="btn" onClick={() => social("google")}>
            ...
            <button type="button" className="btn" onClick={() => social("github")}>
          ...
          {err && (
            <div style={{fontSize: 12, color: "oklch(0.7 0.22 25)", marginBottom: 10}}>
              {err}
            </div>
          )}
